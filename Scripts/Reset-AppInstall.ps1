param(
    ...
    [Parameter(Mandatory = $true)] [string]$RegistryPathsCsv,
...
$registryPaths = $RegistryPathsCsv -split ';' | Where-Object { $_ }
function Invoke-Reset {
    Invoke-Command -ComputerName $ComputerName -ArgumentList $AppGuid,$registryPaths,$RemoteAuditLogDirectory,$RemoteAuditLogFileName,$ServiceName,$OperationId -ScriptBlock {
        param($AppGuid,$RegistryPaths,$RemoteAuditLogDirectory,$RemoteAuditLogFileName,$ServiceName,$OperationId)
        ...
        Add-Content -Path $auditFile -Value ("{0}`tINFO`tSource=RemoteAction`tOperationId={1}`tTarget={2}`tAction=ResetAppInstall`tGuid={3}`tMessage=Reset App Install gestartet." -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'),$OperationId,$env:COMPUTERNAME,$AppGuid) -Encoding UTF8
        $pathsToRemove = New-Object 'System.Collections.Generic.HashSet[string]' ([System.StringComparer]::OrdinalIgnoreCase)
        function Add-RemovalPath {
            param([string]$Path)
            if (-not [string]::IsNullOrWhiteSpace($Path)) { [void]$pathsToRemove.Add($Path) }
        }
        function Remove-EmptyRegistryParents {
            param([string]$Path, [string]$StopAtPath)
            $stopFragment = if ([string]::IsNullOrWhiteSpace($StopAtPath)) { '' } else { $StopAtPath.ToLowerInvariant() }
            $current = Split-Path -Path $Path -Parent
            while (-not [string]::IsNullOrWhiteSpace($current)) {
                if (-not [string]::IsNullOrWhiteSpace($stopFragment) -and $current.ToLowerInvariant().EndsWith($stopFragment)) { break }
                if (-not (Test-Path $current)) { break }
                $children = @(Get-ChildItem -Path $current -ErrorAction SilentlyContinue)
                if ($children.Count -gt 0) { break }
                Remove-Item -Path $current -Recurse -Force -ErrorAction SilentlyContinue
        ...
        foreach ($basePath in $RegistryPaths) {
            ...
            foreach ($candidate in @(Get-ChildItem -Path $basePath -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.Name -like "*$AppGuid*" })) {
                Add-RemovalPath -Path $candidate.PSPath
            }
        }

        $win32AppsRoot = 'HKLM:\SOFTWARE\Microsoft\IntuneManagementExtension\Win32Apps'
        if (Test-Path $win32AppsRoot) {
            function Test-GrsContainerMatchesAppGuid {
                param(
                    [string]$ContainerPath,
                    [string]$AppGuid
                )

                if ([string]::IsNullOrWhiteSpace($ContainerPath) -or [string]::IsNullOrWhiteSpace($AppGuid) -or -not (Test-Path $ContainerPath)) {
                    return $false
                }

                if ((Split-Path -Path $ContainerPath -Leaf) -ieq $AppGuid) {
                    return $true
                }

                try {
                    $containerProps = Get-ItemProperty -Path $ContainerPath -ErrorAction Stop
                    foreach ($property in $containerProps.PSObject.Properties) {
                        if ($property.Name -in @('PSPath','PSParentPath','PSChildName','PSDrive','PSProvider')) { continue }
                        if ($property.Name -ieq $AppGuid -or ([string]$property.Value) -eq $AppGuid) {
                            return $true
                        }
                    }
                }
                ...
                foreach ($child in @(Get-ChildItem -Path $ContainerPath -ErrorAction SilentlyContinue)) {
                    if ($child.PSChildName -ieq $AppGuid -or $child.Name -like "*$AppGuid*") {
                        return $true
                    }

                    try {
                        $childProps = Get-ItemProperty -Path $child.PSPath -ErrorAction Stop
                        foreach ($property in $childProps.PSObject.Properties) {
                            if ($property.Name -in @('PSPath','PSParentPath','PSChildName','PSDrive','PSProvider')) { continue }
                            if ($property.Name -ieq $AppGuid -or ([string]$property.Value) -eq $AppGuid) {
                                return $true
                            }
            ...
            foreach ($targetingKey in @(Get-ChildItem -Path $win32AppsRoot -ErrorAction SilentlyContinue)) {
                $grsPath = Join-Path $targetingKey.PSPath 'GRS'
                if (-not (Test-Path $grsPath)) { continue }
                foreach ($grsContainer in @(Get-ChildItem -Path $grsPath -ErrorAction SilentlyContinue)) {
                    if (Test-GrsContainerMatchesAppGuid -ContainerPath $grsContainer.PSPath -AppGuid $AppGuid) {
                        Add-RemovalPath -Path $grsContainer.PSPath
                    }
                }
            }

            $reportingRoot = Join-Path $win32AppsRoot 'Reporting'
            if (Test-Path $reportingRoot) {
                foreach ($candidate in @(Get-ChildItem -Path $reportingRoot -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -like "*$AppGuid*" -or $_.Name -like "*$AppGuid*" })) {
                    Add-RemovalPath -Path $candidate.PSPath
                }
            }
        }
        ...
        $deleted = 0
        $orderedPaths = @($pathsToRemove | Sort-Object { $_.Length } -Descending)
        foreach ($path in $orderedPaths) {
            if (-not (Test-Path $path)) { continue }
            Remove-Item -Path $path -Recurse -Force -ErrorAction SilentlyContinue
            if (-not (Test-Path $path)) {
                $deleted++
                if ($path -like '*\Reporting\*') {
                    Remove-EmptyRegistryParents -Path $path -StopAtPath 'software\microsoft\intunemanagementextension\win32apps\reporting'
                }
            }
        }

        Restart-Service -Name $ServiceName -Force -ErrorAction Stop
        Add-Content -Path $auditFile -Value ("{0}`tINFO`tSource=RemoteAction`tOperationId={1}`tTarget={2}`tAction=ResetAppInstall`tGuid={3}`tMessage=GRS/Reporting/Registry-Bereinigung abgeschlossen. Treffer={4}." -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'),$OperationId,$env:COMPUTERNAME,$AppGuid,$deleted) -Encoding UTF8
        "Registry-Bereinigung abgeschlossen. Treffer=$deleted"
    } -ErrorAction Stop
}
try {
    try { $result = Invoke-Reset }
    catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB8PAkjG3n/7C3m
# EbGAazhWW8G56NqOKMvDgvN59ASLYqCCH/UwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# W/GqQowNuNuWRAketmf204LnbBT8YZl2YjQJHQSnLEUwDQYJKoZIhvcNAQEBBQAE
# ggEAMM4bmYMzot7vaxhWTdAXuiGoDVyqkAXc1EaABGZyKG6jJ2sYao7vG/S3a4KW
# 9pJhNsIdNrUyJsx+BWaCm+ezIggHz4kEl2OmBCptCAuYcnaFrROCW9g8G2NM+PPv
# x3RgcAUOL4JOeKN3lZv1TIjgnsbTONIWMVDi1rlKVb+F2SSOFWQJpyOnwVUog689
# BVi04pk3/H6dXpIfgdg3Rxm8AtJVou9cUcQGKcF/tI5/zxaFyzvjzzMadvY/GPbp
# m2nkcYA0wzqkqi2ndTxcXf0Xkakk32orvFhVt6Zw3Zs9/J9+0Z+ZqCoYPVTOulMk
# wY810dwCIeNSr1ZRjelYb2jzmqGCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIB
...
# MTcxNDEzMzJaMC8GCSqGSIb3DQEJBDEiBCAimsCN2cdKpQU7D3B2qVKa+kE8QwA4
# RZuaUqV8WX+OeDANBgkqhkiG9w0BAQEFAASCAgCDEup3ckXyLOVjwe6AbUQp9hui
# 34VfUqTgq2VpTUY7Vl85UC8v6eWw+EadfM3nPIJMu9NelayaEANmZPG2oXtBXKXj
# T9mw5ZYmD8hAY6nVLvcWc6pnYZ0V7SLvI/ILtCtUMEK/SZ3PuB3QxTtpfE/q0i6B
# VjhVWv7ZqV1kJI9+79rHOU0Ej5B/Ys9ZbU4T4TWp/QdhDoggSJfTbSd/NO+egEOx
# DY3oMiL7rT4TD81zM/qi137/n6JwA39oIAXtqpwgnswxSVTFW+qlj7qB+q92TPVD
# 3h5HBQ89iGKViMdxEqhTtHGCbsMALoiVbW4xMXLBBwuogH3JYNKXjdznsPpoTxqv
# F6cBwiW+3vE+Ko9S1IeTREKa0gfLArL3iyWuShWyas1aXcHRfZtECyV5G9FI++Vo
# 5KgyasVrCtkYX5w7kiLJYP5i8ETs5Vf7eHx/Hz34kYLhMXWJYHsY+PPCOOHJHQF/
# NUGHbaqoJdeBxERz58cIagLsfY2h/ih2JIWdbSfKrhTqR+aIaEvA1jEUGwE2aizR
# 8hYV/QudxHm5ssN8RbFekifuZH+N/ZbxqPKR2p3gxhG7YXjHTR19xXbhh07XIDnr
# +QAk42ZB7exVK9lDtJyKWfVIOAWUf8TNWmWHCaBWwQOMDEB3KGySjwyUn8oMDaB7
# yysAqxrr0khxI19RsQ==
