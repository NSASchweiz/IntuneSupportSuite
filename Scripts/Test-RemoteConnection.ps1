...
if ($SimulationMode -eq 'True') {
    Write-Output 'ConnectionMode=Simulation;FallbackPrepared=False'
    exit 0
}
...
try {
    $directResult = Invoke-Command -ComputerName $ComputerName -ScriptBlock { $env:COMPUTERNAME } -ErrorAction Stop
    if (-not [string]::IsNullOrWhiteSpace($directResult)) {
        Write-Output 'ConnectionMode=Direct;FallbackPrepared=False'
        exit 0
    }
}
catch {
...
if ($ConnectionFallback -ne 'True') {
    Write-Error 'Direkte PSSession nicht erreichbar und ConnectionFallback ist deaktiviert.'
    exit 1
}
...
try {
    ...
    if (-not $testResult.Success) {
        throw "PSExec Verbindung fehlgeschlagen: $($testResult.StandardError)"
    }
    ...
    $remotingResult = Invoke-Command -ComputerName $ComputerName -ScriptBlock { $env:COMPUTERNAME } -ErrorAction Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC6DiAdXPBBN7p/
# zbt2cAQDjqdhRM6SstXg93B1M3HVNqCCH/UwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# Wf7m92Ao9FlptkbCNJ9+THXkDy6ZDguc5xaqoVoGPzcwDQYJKoZIhvcNAQEBBQAE
# ggEAmdc7aEKfp4FbFv+R/lBbTV/EAujYxqv6dbrj4MnFoYkca+zUfIc0bImEuDVk
# DzcyvdpbLU6rGmFcXlJOYao9Myw6DJvXJHiG5h+LfUHE4YVXH5HVekaGzHzunHo1
# jE833MXt+F163beFE7jcv9vLUNLoFaWtans9/VY0MVDsEBbxkATrTuwBR1ES4S2S
# ypQsJyQlxaHe5Na3HS12tRdaxyegizmzIRaVtdlxXm2WZYZBKbXAFTIACGXcXtol
# JePMQJ9L8bTqj8PyfPTcYbSWzqk3I5orSS8dcx90ghZbPkj7XYtcOA0EeN9Z9hBp
# 0QmpMcBLC6tE8dZx18LGvyFH7aGCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIB
...
# MTcxNDEzMzJaMC8GCSqGSIb3DQEJBDEiBCDhEztmPrOMznsBT5841a2Z3tKQsv5b
# DE9UTKeIW8daJTANBgkqhkiG9w0BAQEFAASCAgBN4z5s/KraYJzSVHQUEulgEBex
# GMUXPWwNuEH39UAGLasasjks4yuO42A9F+nRqbGCHVmh4a3O7xsI9/yr/rwh58l8
# yT0wE1z/2LlmDqeBp4ogeYHvmq7o8M42noEYZLZ/KCtdGA+XgLKodX3OUvQcXdfI
# XlccX9TqpJpT/lU/hFpct4wQgnw8lPYSMhHUfQaDbRQkJcoyJ/vyYk3YPG3ivc7k
# TDvyt0aHQyRupg5F+xhjHfYg2KVdMaBpw3QWkfoc4KI0CuVFSxm5hFhRffq9zkcR
# Y9scGZ2VRablckBoMvwr1or/aMtwx69wHFbADVi3GbATnojxRyEtKWMRggwCR5/h
# gMPvR9ixPUctVUIJwdh8xsCIseoSuwr1aft8+NxEurpk3wkhZFEThi5JlvEAE+GO
# oLTidRDEY48kHOsQy5hO6a6ogJuQ6AEBN4u4S0HFhupbSZoXzKBIiupLJrv0ga+1
# CoT8SNvH5yI0U1hvdlE/4II++R+LgfqzWGiqq5z9ROlZsHdicur0whC+3oEWTK78
# tCHd7fgg2VuAW7Q/hCudpGg683YlS1H4WhhfObuSxLKMkb7mqeyMly309Stl2OUk
# 3ZwF3plw7DJQdwole6XTVLpUloQrmWsidEfC1qa6aF/WybCrOM5LB3wOPf1K/4uI
# bXEGD2UKo7LjaQZRrw==
