...
function Invoke-Reset {
    Invoke-Command -ComputerName $ComputerName -ArgumentList $ImeLogPath,$RemoteAuditLogDirectory,$RemoteAuditLogFileName,$ServiceName,$OperationId -ScriptBlock {
        param($ImeLogPath,$RemoteAuditLogDirectory,$RemoteAuditLogFileName,$ServiceName,$OperationId)
        ...
        Add-Content -Path $auditFile -Value ("{0}`tINFO`tSource=RemoteAction`tOperationId={1}`tTarget={2}`tAction=ImeLogReset`tMessage=IME Log Reset gestartet." -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'),$OperationId,$env:COMPUTERNAME) -Encoding UTF8
        Stop-Service -Name $ServiceName -Force -ErrorAction Stop
        $archive = Join-Path $ImeLogPath ('IME-Log-' + (Get-Date -Format 'yyyyMMddHHmm'))
        New-Item -Path $archive -ItemType Directory -Force | Out-Null
        Get-ChildItem -Path $ImeLogPath -Filter '*.log' -File -ErrorAction SilentlyContinue | Move-Item -Destination $archive -Force
        Start-Service -Name $ServiceName -ErrorAction Stop
        Add-Content -Path $auditFile -Value ("{0}`tINFO`tSource=RemoteAction`tOperationId={1}`tTarget={2}`tAction=ImeLogReset`tMessage=IME Logs wurden archiviert und der Dienst wurde neu gestartet." -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'),$OperationId,$env:COMPUTERNAME) -Encoding UTF8
        'IME Logs wurden archiviert und der Dienst wurde neu gestartet.'
    } -ErrorAction Stop
}
try {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD9npVGGIxsMQgi
# 4V0SeeEhXeyavgqejEIy9ak9/I5sq6CCH/UwggWNMIIEdaADAgECAhAOmxiO+dAt
...
# Fk1Bb2vjhGRF+TGTg7hKbFgURtQCdMqZXIIyekBlObkwDQYJKoZIhvcNAQEBBQAE
# ggEAHmqz12I2ewYbuHL+M3aVUJ0xJAQK+fmbd8W0MkiTGWIpEGgR78AalNcM88yr
# 73mKpw8Wc7MDSndUYGcYzkLLJEBRI5EbAFmagZwudZljDKB8Y/xkqQ0Wyufa1C1r
# EsAD7/QEknS43IGGcxjmYatl8n9Gtkc7oa5PEYoI8m5J7vS/4LVSqYNoWRitHbox
# 40+rjafCZXwstvqKSin1bJe2kHmXMDFOrLvp4xIdjVaUomswI3AQRqMvd9euPsbI
# bzjBtOwzCVE8WmgyJAkqKVjYiFipSp8HsGiLIGe4WOZqQZuLcgEcyuqjTsMWV0xh
# jMeJzDGm4C/exHaYPJg7Wrj3K6GCAyYwggMiBgkqhkiG9w0BCQYxggMTMIIDDwIB
...
# MTcxNDEzMzJaMC8GCSqGSIb3DQEJBDEiBCA5uhUpB7A7+5Aa9IAsQrD5UbUppWgZ
# Z3qZCyifYxqZJzANBgkqhkiG9w0BAQEFAASCAgAIXheSlAPTQsT7bqu2lKgG/2Y9
# YgQfbS9ZKyju/FaOlKHcG4kq53VqQYl2e/Lp0f68GaR3uKFn9BpsXReEgh7K3y/c
# HHe/bqrAnzjNf/XsO7EDClvwrdg2cBaiq5+9CKL9oGSSGUWU4nUmdPF+2qRejKMV
# YuP8Iu4iV3pWv5NL4Dq3lZIMdXOQo29AqJfyWQTVNJxzVEYcOaWyZboTFniBZBWe
# BFY0RU1DJA6xePRgFR7LGJRM0JDVtyoDQBY3slUuAhaCtOWGBMzBsV0RLrJYdGJh
# K7pfkJ6tN+rO5aa65rcWebZZr8lMWpomSa8lPJcWN4FpkbljxNRHEBRvPzS691EH
# qGXxDQVZA4bDpXmCIFrrWt3FH+7YKy1AJ9aJV9IbdscDZkwRXWfM/lxh/EBJUpOL
# o9JJhvZVwhw7o9ZZLmGpW4K14H8E84lA0rF40TDcERZn9zep22Vm5j3FElFH1FUT
# noMiqZuyZUcx2vKd6olaOOwH7O7WQch6dSQNGOtjmgTiaQA6Uw0TmLHSy01PgDRJ
# +RY1gY1DmJsPAbv8VsiWeyCQiscpiYLSX1CocAlVsXm82fcqzARLWH3zM+8OA3f1
# PHmPk5ske2WyJ+trAbeDcoz8tST8f9+WpjsdMI89ZgMd0XbPmTawai0ePffyLwkF
# ULBtl8aIyz6pTl5cXg==
